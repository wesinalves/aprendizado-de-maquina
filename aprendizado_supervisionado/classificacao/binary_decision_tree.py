'''
Algoritmo árvore de decisão binária usando Python
...
reference:
https://github.com/AssemblyAI-Examples/Machine-Learning-From-Scratch/blob/
main/04%20Decision%20Trees/DecisionTree.py
...
class DecisionTree:
    ...
    def _entropy(self, y):
        hist = np.bincount(y)
        ps = hist / len(y)
        e = -np.sum([p * np.log(p) for p in ps if p > 0])
        return e
